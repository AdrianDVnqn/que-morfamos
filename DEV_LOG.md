...
15. **Endurecer los grants de PostgREST.** *(nuevo, 01-sep)* Supabase otorga por defecto a `anon`/`authenticated` grants completos (SELECT, INSERT, UPDATE, DELETE, TRUNCATE) sobre todo el esquema `public`. Hoy lo único que impide usar los de escritura es RLS: las políticas sólo permiten SELECT, así que las escrituras quedan bloqueadas por **ausencia** de política. Funciona, pero es una sola línea de defensa — si alguien agrega después una política `FOR ALL` en vez de `FOR SELECT` (error fácil en el panel), esos grants ya están ahí y pasan a ser usables con la anon key, que es pública por diseño.
    - **El script está escrito y revisado: `hardening_grants.sql`.** Va a mano en el SQL Editor y no como script ejecutable porque es un cambio de privilegios en producción. Incluye verificación y revert.
    - ⚠️ **NO revocar el `SELECT`.** El dashboard lee por PostgREST con la anon key desde el cliente, y en Postgres RLS restringe lo que un rol ve pero el rol necesita igual el GRANT de tabla: revocarlo dejaría las políticas de "Lectura pública" sin efecto. Consulta `lugares`, `scraping_logs`, `reviews` y `review_history`.
    - Ni el backend ni el scraper se ven afectados: los dos usan `DATABASE_URL` como rol `postgres`, dueño de las tablas y con `BYPASSRLS`.

#### 🔵 Baja prioridad / housekeeping

16. **`DEEPSEEK_MODEL` por defecto apunta a un ID viejo:** `deepseek-chat` ya no figura en el catálogo de DeepSeek (ahora `deepseek-v4-flash`/`v4-pro`). Sólo importa si se vuelve a ese proveedor; el default está en `main.py` y es overrideable por env.

17. **LangSmith desactualizado.** No se corrió `run_langsmith_eval.py` desde los cambios del 25/26-ago, así que el historial trackeado no refleja el 22/22 actual ni el cambio de proveedor. Correrlo deja la comparación "antes/después" registrada.

18. **~~Minar `query_logs`~~ DESCARTADO por ahora.** *(01-sep)* Hay 1579 filas, pero el usuario confirma que **~95% son pruebas suyas**. Minarlas mediría el comportamiento del desarrollador y no el de usuarios reales, y ese sesgo es **peor que no tener datos**, porque parece evidencia. Retomar cuando el sitio tenga tráfico genuino, filtrando por IP o sesión antes de mirar nada.

19. **Tab de LangSmith en el dashboard (`que-morfamos-dashboard`).** Evaluado y pospuesto: por ahora alcanza con la UI propia de LangSmith (`smith.langchain.com`, proyecto `que-morfamos`, dataset `que-morfamos-golden` → Experiments). Retomar sólo si se quiere todo centralizado en el portfolio.

20. **~~Housekeeping del repo~~ ✅ HECHO el 01-sep.** Se destrackeó el `.pyc`; `backups/` —que guarda reseñas de usuarios— entró al `.gitignore` junto con `temp.txt` y `.claude/`; y se eliminaron las **452 líneas** de `procesar_restaurante()` en `opiniones-scraper.py` y su copia `-debug`, definida y nunca llamada. Se borró en vez de dejarla "por las dudas": una copia muerta de la lógica de scraping invita a arreglar bugs en el lugar equivocado, que es exactamente lo que pasó el 28-ago. Verificado además que no haya secretos trackeados y que `mis_claves.env` esté en `.gitignore` en los tres repos.
