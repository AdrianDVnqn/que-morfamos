...
- **Tipo de aserción `expected_category`:** para queries de categoría ("mejores pizzerías"), chequear que las cards devueltas tengan la `categoria` correcta (ej. ≥4 de 5 con `categoria ILIKE '%Pizza%'`) en vez de compararlas contra una lista subjetiva de nombres. Es el fix pendiente de `typo_sin_acentos_pizza`.
- **Arreglar el bug que `sin_resultados_edge` ahora expone:** el RAG devuelve 5 restaurantes reales para "restaurantes de comida marciana en la luna" en vez de admitir que no encontró nada.
