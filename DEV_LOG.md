...
6. **UX/UI en mobile: el header entero queda fuera de la pantalla y no hay forma de llegar a él.** *(nuevo, 01-sep — reportado por el usuario, auditado a 375×812)* El reporte fue "anda medio mal en cuanto a márgenes y scrolls"; midiéndolo apareció algo bastante peor que un problema de márgenes:
   - **El banner superior está renderizado arriba del viewport y es inalcanzable.** `.header-video-wrapper` (el cartel de neón) queda en `y = -121` con 120px de alto; los tres botones de tono (Cordial / Soberbio / Irónico) en `y = -79`; el de silenciar en `y = -78`. **Y la página no scrollea** (`scrollHeight` 812 = `innerHeight` 812), así que no hay gesto que los traiga. Están en el DOM —por eso los lee un lector de pantalla y aparecen en el árbol de accesibilidad— pero son inusables: **el selector de tono, que es una feature del producto, no existe en mobile.**
   - **Blancos de toque por debajo del mínimo.** Botones de tono 45×36, silenciar 34×34, chips de ejemplo 41px de alto. La referencia (Apple HIG / WCAG 2.5.5) es 44×44.
   - **Los chips de ejemplo scrollean en horizontal sin ninguna señal de que se pueda** (`scrollWidth` 789 sobre `clientWidth` 259): se ven cortados al medio y parecen rotos. Un scroller horizontal anidado dentro de una página que scrollea en vertical, además, es de las interacciones más molestas en touch.
   - **`.chip-bar-mobile` y el formulario se solapan 9px.**
   - **Lo que sí está bien** y conviene no romper: el input tiene `font-size: 16px`, que es lo que evita que iOS haga zoom al enfocarlo, y no hay desborde horizontal de la página.
   - ✅ **Ya resuelto en esta pasada:** el pie se comía 117px de los 812 (14% de la pantalla) mostrando fechas que se miran una vez. Ahora se pliega en un `<details>` en mobile y el disparador comparte fila con el crédito: **117px → 71px**. En desktop se ve igual que antes.
   - ⚠️ **Alcance de la auditoría:** cubre **sólo la pantalla de inicio**. La vista de resultados (cards + mapa + modal de detalle) no se pudo medir porque el pane del browser se colgaba al enviar la consulta — y es justo donde el usuario dice que están los problemas de márgenes y scroll. **Falta auditarla.**
