...
6. **UX/UI en mobile: la pantalla está mal repartida y el header de la home queda fuera de vista.** *(nuevo, 01-sep — reportado por el usuario, auditado a 375×812 en las dos vistas)* El reporte fue "anda medio mal en cuanto a márgenes y scrolls". Medido, el problema principal es **cuánta pantalla se le da al contenido**:
   - **Vista de resultados — el chat vive en una mirilla.** `.messages-container` mide **276px de los 812** (34% de la pantalla) y contiene **2452px** de conversación: una relación de 9:1. El resto se lo llevan el header (120px), el formulario (82), el pie (86) y la barra de pestañas (60).
   - **204px de huecos muertos, un 25% de la pantalla**, repartidos en tres: **98px** entre la barra de tono y los mensajes, **85px** entre el formulario y el pie, y **21px** entre el pie y la barra de pestañas. Eso solo ya duplicaría el alto del chat.
   - **Vista de "Lugares" — una card no entra en su propio contenedor.** `.cards-grid` da 355px de alto para 1935px de contenido, y **una sola card mide 370px**: nunca se ve una card completa sin scrollear. El pie y la barra de pestañas se comen 146px fijos abajo en ambas vistas.
   - **En la pantalla de inicio el header está renderizado ARRIBA del viewport y es inalcanzable.** `.header-video-wrapper` (el cartel de neón) en `y = -121` con 120px de alto, los tres botones de tono en `y = -79`, el de silenciar en `y = -78`. Y la página no scrollea (`scrollHeight` 812 = `innerHeight` 812), así que no hay gesto que los traiga. **Esto pasa sólo en la home:** con resultados en pantalla (`.App.sidebar-layout`) el header aparece normal en `y = 11` y los botones de tono son visibles y usables. O sea que el selector de tono existe, pero no antes de la primera búsqueda — justo cuando se elegiría.
   - **Blancos de toque por debajo del mínimo** (referencia Apple HIG / WCAG 2.5.5, 44×44): botones de tono 45×36, silenciar 34×34, chips de ejemplo 41px de alto.
   - **Los chips de ejemplo scrollean en horizontal sin ninguna señal de que se pueda** (`scrollWidth` 789 sobre `clientWidth` 259): se ven cortados al medio y parecen rotos.
   - **`.chip-bar-mobile` se solapa 9px con el formulario.**
   - **Lo que está BIEN y conviene no romper al tocar esto:** no hay desborde horizontal en ninguna de las dos vistas; el input tiene `font-size: 16px`, que es lo que evita que iOS haga zoom al enfocarlo; y las cards y el mapa **sí son alcanzables** por una barra de pestañas propia de mobile (💬 Chat / 🍽️ Lugares / 🗺️ Mapa), que es una solución correcta y no hay que rehacerla.
   - ✅ **Ya resuelto en esta pasada:** el pie mostraba las fechas del dataset siempre, comiéndose 117px de los 812 (14% de la pantalla) para un dato que se mira una vez. Ahora se pliega en un `<details>` en mobile, con el disparador compartiendo fila con el crédito: **117px → 71px**. En desktop se ve igual que antes.
   - **Por dónde empezar:** los 204px de huecos son la ganancia más grande y la más barata — es puro CSS y no cambia ninguna interacción. Después, decidir si el pie tiene que estar visible en la vista de resultados, donde compite con la barra de pestañas.

7. **El contador del popup "despertando el servidor" se reinicia solo, y aparece en el momento equivocado.** *(nuevo, 01-sep — reportado por el usuario, causa CONFIRMADA)*
   - **Síntoma:** el contador va `1, 0, 1, 0, 1, 2, 3, 0…` en vez de subir.
   - **Causa:** los dos efectos que manejan estos modales (`src/App.js`, el de `apiStatus === 'error'` y el de `'checking'`) llevan **`messages` en el array de dependencias**, y en el cuerpo hacen `setBackendConnectingSeconds(0)` / `setBackendCountdown(60)`. La escena de bienvenida agrega los mensajes mockeados de a uno con temporizadores (`setMessages(prev => [...prev, m])`), así que **cada mensaje que aparece cambia la identidad del array, vuelve a correr el efecto, limpia el `setInterval` y pone el contador en cero**. Como los mensajes llegan a intervalos irregulares, el patrón parece aleatorio: a veces llega a 3 antes de que caiga el siguiente. `messages` sólo se usa para calcular el booleano `isInitialPage`, pero se depende del array entero.
   - **Fix candidato:** que el efecto dependa del booleano derivado y no del array (o separar el intervalo del contador en su propio efecto, que sólo dependa de si el modal está visible).
   - **Y un cambio de producto, pedido por el usuario:** el popup debería aparecer **cuando el usuario hace click en la barra para escribir**, no al entrar a la página. Hoy interrumpe antes de que haya intención de usar nada.

8. **El chat de bienvenida se rompe si el usuario escribe antes de que termine.** *(nuevo, 28-ago — frontend)* Reportado por el usuario y reproducido de casualidad durante la verificación de esta misma sesión: si se manda una consulta mientras la escena mockeada de WhatsApp todavía se está tipeando, el mensaje real del usuario queda **intercalado entre los mensajes inventados** y el del Sommelier se **fusiona** con el que sigue. La escena se agenda con una cadena de timers en el `useMemo`/efecto de `BIENVENIDA` (`src/App.js`) que sigue empujando mensajes al mismo array aunque ya haya conversación real. Fix candidato: cancelar la secuencia pendiente en cuanto el usuario envía (ya existe la bandera `cancelado` en el efecto, falta dispararla desde el submit) y decidir si los mensajes mock ya emitidos se limpian o se congelan.

9. **`exactos = locales_verificados[:3]`** (`main.py`, en `procesar_consulta_gen`) toma los 3 primeros aprobados por el Juez **en orden de lista, no por relevancia**. Se mitigó indirectamente mejorando el orden de `grupo_alta`, pero la causa sigue: si el Juez es permisivo, un lugar popular pero tangencial le gana el lugar a uno más específico. Confirmado con experimento controlado que es **preexistente**, no introducido por los fixes de esta sesión. Fix candidato: ordenar `locales_verificados` por alguna señal de calidad/especificidad antes de cortar a 3.

10. **El orden de "sin tacc" no aplica la clave de relevancia.** *(nuevo, 28-ago — síntoma medido, causa SIN confirmar)* Buscando "sin tacc" sale Antares (4.2★/5027) primera y Lucciana Pastelería Sin Gluten (4.7★/60) tercera. Calculando la clave a mano debería ser al revés: Lucciana tiene evidencia 3 y Antares 1. El docstring de `relevancia()` usa **ese mismo caso** como ejemplo de lo que arregla. Sospecha: `usa_match_count` queda en `False` cuando `es_query_generica` es `True`, y ahí `grupo_alta` se ordena por `calc_score` (popularidad pura). No se llegó a confirmar.

11. **Los ítems de "Lo mejor" se repiten entre sí.** *(nuevo, 28-ago)* Reportado por el usuario con un caso claro: para Cabildo Pizzería los tres bullets dicen lo mismo — "Excelentes pizzas!!!", "Pizza es esponjosa 100% y le colocan suficientes ingredientes", "Unas pizzas galácticas imperdibles". Tres formas de decir "las pizzas son ricas", cuando el bloque debería cubrir aspectos **distintos** (comida, ambiente, atención, precio).
