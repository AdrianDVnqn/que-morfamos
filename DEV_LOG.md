...
## 📅 Sesión: 1-2 de Septiembre de 2026 (Resúmenes v5/v6, la métrica autorreferencial, y el pipeline congelado)
...
Mover la métrica de evidencia, clavada en 0.53. Terminó destapando que **el pipeline de resúmenes




no corría desde julio** y que **la métrica se podía inflar sola**.
...
### 🔴 El hallazgo más grave: los resúmenes llevaban 7 semanas congelados




El log de la corrida semanal mostraba "no hubo cambios" y terminaba en verde. La causa NO era el




saldo agotado de DeepSeek —eso se diagnosticó mal primero—: la corrida ni llegaba a llamar al LLM.




929 lugares en 2m48s, cero `[CHECK]`.
...
- **La ruta incremental exigía 20 reseñas nuevas VÁLIDAS** por lugar. Medido: de 245 lugares con




  reseñas nuevas en la semana, los que llegaban a 20 eran **cero**.




- **Y la ventana se reiniciaba sola.** Cada lugar que no alcanzaba el umbral igual llamaba a




  `actualizar_resumen_lugar(nombre, resumen_actual)`, que pone `embedding_updated_at = NOW()`




  aunque el resumen no cambie. Como la ventana se mide contra esa fecha, las 3 reseñas de esta




  semana no se sumaban a las 4 de la próxima. **Era una condición imposible de cumplir.**
         ...
         umbral 15    ventana que se reinicia: 3 lugares/semana | acumulando: 119




         umbral 20    ventana que se reinicia: 0                | acumulando:  86
...
- **El umbral nuevo salió del corpus, no de la intuición.** La mitad de las reseñas no tiene texto




  (percentiles 10 y 25 = 0 caracteres); de las que sí, **el 38% menciona algún concepto**. Con




  p=0.38, cinco reseñas dan 1.9 conceptos esperados — el mismo listón de ≥2 que usa la métrica de




  evidencia. Se fijó en **5**.




- Se corrió a mano: **100 lugares regenerados, ~$0.05**, y el LLM dijo "hay info nueva" en los 100.




  Quedan 190 de backlog que se drenan solos.
...
### 🔬 La métrica de evidencia era autorreferencial




Leía el mismo campo que usa el ranking, así que cualquier experimento que agregara texto la




inflaba. La columna v3 medía **0.77 contra sí misma y 0.57 contra una vara fija**. Ahora la vara




son las **reseñas** (≥2 menciones positivas) y la calidad del resumen se reporta aparte como




*cobertura*. Detalle completo en `ARQUITECTURA_RAG.md`.
...
### 🧪 Resúmenes: v5.1 sirve, v6 no, v3 es lo mejor medido




| | pasan | Recall | Precision | Evidencia | Cobertura |
...
| v1 (producción) | 27/33 | 0.80 | 0.84 | 0.51 | 0.88 |




| v5.1 | 28/33 | 0.74 | 0.73 | 0.58 | 0.90 |




| **v3 (v1+v5.1)** | **30/33** | 0.79 | 0.80 | 0.57 | **0.97** |
...
v3 arregla `pizza sin tacc`, `sushi vegano` y `lugares románticos` sin romper ninguno. **No se




promovió** porque sostener esa forma exige generar DOS resúmenes por lugar, y **v6 —un solo prompt




que hiciera ambas cosas— fracasó**: retrocedió en los dos ejes a la vez (recall de features 51%→41%,




frases de ausencia 12%→27%). Al darle presión descriptiva, el modelo afloja la disciplina del




checklist. Concatenar dos generaciones especializadas le gana a pedirle una sola que haga ambas.
...
### 🐛 Bug de ranking encontrado de rebote




`_mencion_positiva` trataba el `"sin"` de *sin gluten* igual que el de *sin pelotero*: un resumen




diciendo que el lugar **NO** tiene algo contaba como que **SÍ** lo tiene. Acotado a `FEATURES_CON_SIN`.
...
### ⏱️ Scraper: medido, y una propuesta retirada




    NUEVAS_REVIEWS   299 lugares   19.5 s c/u   97 min   (52%)




    SIN_CAMBIOS      609 lugares    7.9 s c/u   80 min   (43%)
...
- **La detección de cierres vive en el camino de 7.9s** y nunca entra a Opiniones. Eso **descarta




  la cadencia adaptativa** que se había propuesto (saltear lugares quietos): apuntaba a lo barato y




  encima habría pagado con latencia de detección de cierres. **No reintentarla.**




- Se sacó un `sleep(2)` fijo de la cabecera (31 min/corrida) reemplazándolo por espera por




  condición sobre los DOS elementos que se leen después — esperar sólo `div.F7nice` dejaba al




  rating sin garantía. **Instrumentado**: si Google cambia el DOM, el contador "Cabecera sin




  confirmar" lo delata en el resumen y en Discord.




- El piso de reseñas a cargar bajó de 20 a 10: la mediana de los que cambian es **2**.




- **Retorno decreciente**: lo que queda son sleeps fijos en la navegación a Opiniones, sin forma de




  validarlos sin correr Selenium. Y el repo es público, así que los minutos son gratis. El próximo




  paso con retorno real es la **región de la base** (pendiente #2), que afecta a producción y no
...
### 🎯 Ranking: dos reportes del usuario, dos causas distintas

**"pizzas estilo napoletano" devolvía lo mismo que "mejores pizzas".** El router entrega
`keywords=['pizza']` a secas: el modificador se pierde **antes** del ranking, y `atomizar_keywords`
no puede recuperarlo porque sólo parte lo que recibe. Se agregó `rescatar_conceptos_de_la_query()`,
que busca en la consulta los términos del vocabulario que ninguna keyword cubre.

> **La distinción que lo hace seguro:** el rescate agrega **modificadores**, nunca tipos de comida.
> El router identifica QUÉ quiere el usuario; el rescate agrega CÓMO o CON QUÉ. La primera versión
> no tenía ese filtro y rompió `no_falso_bloqueo_queja`: para *"la milanesa que comí ayer estaba
> una mierda, recomendame otra parrilla buena"* rescataba **"milanesa"** — justo lo que el usuario
> descarta. Lo detectó el benchmark.

**"mejores pizzas" devolvía 2 brewpubs** (Antares 4.2/5027 y OGHAM 4.2/4530) compitiendo con
Cabildo Pizzería (4.2/4542). El rating de una cervecería habla de su cerveza, no de sus pizzas.

- **Primero se probó cambiar `calc_score` por un promedio bayesiano** con piso de credibilidad, para
  que un 5.0 con 20 reseñas no ganara por default. **El benchmark lo rechazó sin ambigüedad:
  `29/33 → 23/33`, `Recall 0.80 → 0.56`.** Revertido. La razón la aportó el usuario: el volumen de
  reseñas no es sólo popularidad, también dice que el lugar es **relevante y está establecido**.
- **La causa real era otra:** un rating alto pesa igual venga de una pizzería o de una cervecería
  que además vende pizza. Eso es **especificidad de categoría**, y se arregla mirando `categoria`.
  El orden pasó a ser `(coincide_categoria, calc_score)`.
- El bonus **no** aplica a requisitos excluyentes (vegano, sin tacc) aunque estén en
  `KEYWORD_TO_CATEGORIES`: ahí la categoría miente, y el código ya documentaba el caso — BIO ZEN
  figura como "Restaurant" pese a ser vegetariano.

**Estado final del motor:** `29/33 | Recall 0.75 | Precision 0.78 | MRR 1.00 | Evidencia 0.65 |
Cobertura 0.94`. El MRR llega a 1.00 —el primer resultado es siempre el correcto— y la evidencia
sube, a cambio de ~0.05 de recall. Se aceptó el intercambio a conciencia; el pendiente #6 propone
resolverlo mejor repartiendo el top 5.

### 🧭 Qué se descartó hoy, con números
Vale tanto como lo que entró, porque evita reintentarlo:
- **Promover v3** (resúmenes concatenados): empataba en casos y perdía recall/precisión una vez que
  los datos estuvieron consistentes. Buena parte de su ventaja aparente era que los resúmenes
  estaban viejos.
- **Prompt v6** (uno solo que describa Y enumere): retrocedió en los dos ejes a la vez.
- **Promedio bayesiano**: `29/33 → 23/33`.
- **Cadencia adaptativa del scraper**: apuntaba al camino barato y habría pagado con latencia de
  detección de cierres.

### 🔎 Transparentar por qué aparece cada lugar

Las tarjetas del chat ahora muestran **qué conceptos de la consulta cumple cada local**
("Cumple: parrilla, pelotero") y su **QM Score en escala 0-5**, al lado de las estrellas de Google.
Un ícono (i) en el encabezado abre el criterio completo.

**Por qué los chips y no sólo el número.** El QM Score explica muy poco del orden: el ranking es
una cascada `(conceptos, evidencia, QM)` y el QM es el **último desempate**. Mostrarlo solo
prometería explicar el ranking y lo explicaría mal. El caso que lo deja a la vista, medido en
"parrilla con pelotero":

| lugar | Google | QM | conceptos | posición |
|---|---|---|---|---|
| 827 Punto de encuentro | 3,3 ⭐ | 2,9 | parrilla + pelotero | **3.º** |
| El Boliche de Alberto | 4,1 ⭐ | **4,3** (el más alto) | sólo parrilla | **último** |

Sin explicación, ver un lugar de 3,3 estrellas arriba de uno de 4,1 parece favoritismo. Por eso el
panel dice explícitamente que nadie paga por aparecer.

**Una sola fórmula.** `calc_score` —el que ordena— pasó a llamar a `qm_bruto()`, a nivel de módulo.
Estaba escrita una vez sola porque sólo servía para ordenar; al mostrarla al usuario habría quedado
duplicada, y duplicarla garantizaba que algún día el número que ve la gente dejara de ser el que
ordena. Los `conceptos_cubiertos` usan el mismo criterio que `relevancia()`, por lo mismo.

**La normalización a 0-5** divide por un techo **teórico** fijo (16,6), no por el máximo observado:
si fuera el máximo real, el QM de un local cambiaría porque *otro* local creció. Se evaluó
normalizar por percentil y se descartó con datos: dejaba a Cabildo Pizzería en QM 2,0 y a Amore
Pizza Napoletana en 1,0. Un percentil no se lee como una calificación.

### 📊 Paneles con la consulta SQL a la vista

Página nueva en el dashboard con cinco paneles, cada uno con su SQL desplegable: top 5 por QM
contra top 5 por rating de Google (lado a lado, para mostrar la tensión), top 5 con más
movimiento, locales que dejaron de responder (usa `URL_MUERTA` del scraper) y cobertura del
pipeline.

**El SQL no está escrito en el front:** sale de `dashboard_definiciones`, que lo obtiene con
`pg_get_viewdef()`. Copiarlo a mano lo habría condenado a desincronizarse de la consulta real —el
mismo problema que la fecha de corte duplicada—, y un panel que muestra una consulta que no es la
que corrió es peor que no mostrar ninguna.
...
`query_logs` estaba expuesta por PostgREST con RLS apagado —**y con grants de escritura vivos**—




sobre 1578 consultas reales. Corregido. `hardening_grants.sql` queda para correr a mano.
...
1. **Intentar una v5 de resúmenes que gane en retrieval, no sólo en honestidad.** Producción sigue con los resúmenes v1, que tienen **33 afirmaciones inventadas, 127 truncados a mitad de oración y 51 sin el párrafo de características** — pero v4 (que corrige todo eso) mide peor en retrieval y por eso no se promovió (ver la sección de las 4 iteraciones, arriba). Toda la infraestructura ya está construida y funcionando: `regenerar_resumenes.py` con shadow write, versionado por `resumen_prompt_version`, muestreo por característica, chequeo de groundedness, y `RESUMEN_COLUMN` en el backend para evaluar sin promover. Bumpear `PROMPT_VERSION` a v5 y regenerar cuesta ~$0.33 y ~13 min.
   ...
   - ✅ **v5 probado sobre muestra dirigida el 01-sep, con resultado positivo. Falta la corrida completa y el benchmark.**





     - **La hipótesis se confirmó con números.** El resumen se come entre un tercio y la mitad de las características que las reseñas SÍ confirman, y el resumen es la única evidencia que lee el ranking. Cobertura sobre lugares con ≥2 reseñas que mencionan la feature: `pet friendly` 54%, `wifi` 43%, `música en vivo` 44%, `terraza` 63%, `sin TACC` 63%, `vegano` 77%. Ningún fix de vocabulario ni de orden puede recuperar un lugar cuyo resumen omitió la feature.





     - **Causa:** v4 sólo tiene un FILTRO ("mencionala nada más si las reseñas la confirman") y ninguna instrucción de RECALL — nunca le pide al modelo que las busque. v5 agrega una lista a REPASAR ítem por ítem, conservando intacto el filtro de confirmación, que es lo que evita que repasar se vuelva inventar.





     - **Resultado sobre 40 lugares elegidos por ser fallos conocidos** (nuevo modo `--faltantes`, que selecciona lugares donde la reseña confirma la feature y el resumen la omite — medir sobre el golden dataset diluye la señal con lugares que no tienen nada que recuperar). Coste: **$0.021**.
           ...
           recall de features (menciones positivas)   v1 = 33%   ->   v5 = 48%





           sin tacc      30% -> 57%      vegano        32% -> 58%





           pet friendly  36% -> 64%      terraza       16% -> 32%
     ...
     - **El prompt quedó versionado por env var** (`RESUMEN_PROMPT_VARIANT`, default `v4`). Era necesario: `regenerate_embeddings.py` escribe en la columna de PRODUCCIÓN en la corrida semanal, así que iterar el prompt habría cambiado en silencio lo que genera el cron.





     - **Ojo con las "regresiones" aparentes:** `pelotero` figuraba 3/3 en v1 y 0/3 en v5. Mirando el texto, la cobertura de v1 eran **afirmaciones de ausencia** — *"apto para niños, aunque sin pelotero"*, *"no se mencionan servicios como pelotero"*—, o sea exactamente la patología que el prompt v2+ se escribió para eliminar. Que v5 las omita es lo correcto.





     - **Y eso destapó un bug real en `_mencion_positiva`** (ver la sección aparte, abajo).
...
#### ✅ Resuelto el 01-sep: `_mencion_positiva` contaba las negaciones como afirmaciones
...
La excepción que trata a `"sin"` como parte del término —correcta para features que **se llaman** "sin algo" (sin gluten, sin TACC, sin lactosa)— se aplicaba a **todas**:
    ...
    _mencion_positiva("apto para ninos, aunque sin pelotero", "pelotero")  ->  True





    _mencion_positiva("sin estacionamiento propio", "estacionamiento")     ->  True





    _mencion_positiva("ambiente tranquilo pero sin wifi", "wifi")          ->  True
...
O sea que un resumen diciendo que el lugar **NO** tiene la característica lo hacía contar como que **SÍ** la tiene, tanto en el ranking (`conceptos` y `evidencia`) como en las aserciones del benchmark. Y no era un caso de borde: los resúmenes v1 de producción enumeran características en negativo para casi todos los lugares.
...
Arreglado acotando la excepción a `FEATURES_CON_SIN` (gluten, tacc, lactosa, azúcar, alcohol, conservantes, sal). Verificado con 10 casos, positivos y negativos. El benchmark no se movió (`27/33 | 0.83 | 0.84 | MRR 0.97`, evidencia 0.53): las consultas que fallan hoy no son de las afectadas por este patrón. Es un fix de corrección, no de métrica.
...
6. **Mezcla de "puros" y "mencionados" en el top 5.** *(nuevo, 02-sep — propuesto por el usuario)* Hoy el orden pone la categoría por encima de la popularidad, así que para "mejores pizzas" salen 5 `Pizza restaurant`. Resuelve el problema reportado —salían 2 brewpubs— pero es un criterio **binario**, y el usuario señala el costo: *"puede haber lugares que venden más cosas pero que su pizza es excelente, y no debería hacer que pierdan"*.


    - **La tensión está medida.** Al activar la categoría: `MRR 0.97 → 1.00` (el primer resultado es siempre el correcto) pero `Recall 0.80 → 0.75` y `Precision 0.82 → 0.78`. O sea que acierta más arriba y trae menos de la lista curada — que es exactamente lo que se esperaría si estuviera dejando afuera lugares buenos que no son "puros".


    - **La propuesta:** en vez de elegir un criterio, repartir el top 5. 2 o 3 lugares **del rubro** (categoría coincidente) y 2 o 3 donde **lo pedido esté muy mencionado en las reseñas**, que es como venía funcionando.


    - **Encaja con lo que ya existe:** el armado de cards ya es `exactos[:3] + relacionados[:2]`. La mezcla sería alimentar cada mitad con un criterio distinto, no inventar una estructura nueva.


    - **Qué habría que medir para validarlo:** si el recall se recupera hacia 0.80 **sin** perder el MRR 1.00, la mezcla es estrictamente mejor que cualquiera de los dos criterios solos. Si el MRR baja, hay que revisar cuál mitad va primero.


    - **Ojo con el dataset:** parte de la caída de recall puede ser que las listas curadas se armaron cuando los brewpubs SÍ rankeaban en consultas de pizza. Antes de leer el recall como pérdida, revisar caso por caso si el lugar "perdido" era realmente una buena respuesta — ya pasó con `nombre_parcial_growler`.





7. **~~Escribir `ARQUITECTURA_RAG.md`~~ ✅ HECHO el 01-sep.** Está en la raíz del repo. Cubre el recorrido completo en 12 secciones, con las referencias al código **verificadas una por una** contra `main.py`, y cierra con cómo se mide todo y cuál es la línea base. Prioriza lo que no se deduce leyendo el código: que `intencion` no mapea 1 a 1 al `mode`, que las dos funciones de expansión usan vocabularios distintos a propósito, y que `evidencia` significa cosas opuestas según el concepto.





    - **Falta mantenerlo:** es un corte transversal, no un diario. Cuando algo cambie se actualiza la sección, no se agrega una entrada. Tratarlo como al DEV_LOG lo devuelve al problema que vino a resolver.
...
10. **~~El chat de bienvenida se rompe si el usuario escribe antes de que termine~~ ✅ HECHO el 01-sep.** La bandera de cancelación ya existía en el efecto, pero **sólo la levantaba el cleanup**, o sea el desmontaje: mientras la página seguía abierta no había nada que detuviera la cadena de temporizadores, así que los mensajes mockeados se seguían agregando después del mensaje real. Ahora `sendQuery` la levanta y apaga el indicador de tipeo. Va en un `ref` y no en estado porque la escena corre dentro de un `async` creado una sola vez: una variable de estado quedaría capturada en el closure con su valor viejo. Se chequea **dos veces por iteración** —antes del indicador y otra vez justo antes de escribir—, porque entre el `await` del tipeo y esa línea el usuario puede haber enviado. Verificado en el navegador enviando en pleno tipeo: la consulta real queda última y no se agrega nada después, esperando 12s.
...
16. **~~Endurecer los grants de PostgREST~~ ✅ HECHO el 02-sep.** El usuario corrió `hardening_grants.sql` en el SQL Editor. Estado verificado desde los dos lados:
         ...
         TABLA                     RLS   GRANTS a anon/authenticated
         ...
         review_history            ON    SELECT



         scraping_logs             ON    SELECT



         validation_reports        ON    SELECT



         query_logs                ON    (sin acceso)



         langchain_pg_collection   ON    (sin acceso)



         langchain_pg_embedding    ON    (sin acceso)
    ...
    - **El dashboard sigue funcionando**: `SET LOCAL ROLE anon; SELECT count(*) FROM lugares` devuelve 929. Esa comprobación era la que importaba, porque el `SELECT` **no** se revoca a propósito: en Postgres RLS restringe lo que un rol ve, pero el rol necesita igual el GRANT de tabla, así que revocarlo habría dejado las políticas de "Lectura pública" sin efecto.



    - El backend sigue escribiendo `query_logs` (2347 filas): usa `DATABASE_URL` como rol `postgres`, dueño de las tablas y con `BYPASSRLS`.
...
17. **`DEEPSEEK_MODEL` por defecto apunta a un ID viejo:** `deepseek-chat` ya no figura en el catálogo de DeepSeek (ahora `deepseek-v4-flash`/`v4-pro`). Sólo importa si se vuelve a ese proveedor; el default está en `main.py` y es overrideable por env.
...
18. **LangSmith desactualizado.** No se corrió `run_langsmith_eval.py` desde los cambios del 25/26-ago, así que el historial trackeado no refleja el 22/22 actual ni el cambio de proveedor. Correrlo deja la comparación "antes/después" registrada.
...
19. **~~Minar `query_logs`~~ DESCARTADO por ahora.** *(01-sep)* Hay 1579 filas, pero el usuario confirma que **~95% son pruebas suyas**. Minarlas mediría el comportamiento del desarrollador y no el de usuarios reales, y ese sesgo es **peor que no tener datos**, porque parece evidencia. Retomar cuando el sitio tenga tráfico genuino, filtrando por IP o sesión antes de mirar nada.
...
20. **Tab de LangSmith en el dashboard (`que-morfamos-dashboard`).** Evaluado y pospuesto: por ahora alcanza con la UI propia de LangSmith (`smith.langchain.com`, proyecto `que-morfamos`, dataset `que-morfamos-golden` → Experiments). Retomar sólo si se quiere todo centralizado en el portfolio.
...
21. **~~Housekeeping del repo~~ ✅ HECHO el 01-sep.** Se destrackeó el `.pyc`; `backups/` —que guarda reseñas de usuarios— entró al `.gitignore` junto con `temp.txt` y `.claude/`; y se eliminaron las **452 líneas** de `procesar_restaurante()` en `opiniones-scraper.py` y su copia `-debug`, definida y nunca llamada. Se borró en vez de dejarla "por las dudas": una copia muerta de la lógica de scraping invita a arreglar bugs en el lugar equivocado, que es exactamente lo que pasó el 28-ago. Verificado además que no haya secretos trackeados y que `mis_claves.env` esté en `.gitignore` en los tres repos.
